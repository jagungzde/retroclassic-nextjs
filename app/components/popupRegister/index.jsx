    ...
    auth.register(params)
      .then((res) => {
        if (res.flag == 1) {
          ...
          setMessage({status: "error", "message":"Register Failed. "+ res.data});
    ...
    >
      <div className="w-full max-w-[500px] bg-[#222222] mt-30 shadow-2xl m-auto py-4 px-10" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="text-lg font-bold pb-5">
          Create your Perfect World Retro Classic Account
        </div>
        <div className="field">
        ...
        <div className="field">
          ...
          {phone == "" && (
            <div className="error">Phone number must filled</div>
          )}
        </div>
        <div className="field">
          <div className="text-center" id="captcha-container"></div>
        </div>
        <div className="mb-4 text-white hover:text-[#ffe488] hover:underline hover:cursor-pointer" onClick={() => onClose('resend')}>
        ...
        <div className="field-button">
            ...
            onClick={doSubmit}>Sign Up</button>
        </div> 
      </div>
    </div>
    </>
